-- Comprehensive enhancements for JijiSauti platform

-- Add location picker support and anonymous reporting to civic_issues
...
-- Add coordinates support to zoning violations (assuming table exists)
...
-- Enable RLS on zoning violations
ALTER TABLE zoning_violations ENABLE ROW LEVEL SECURITY;

-- Create policies for zoning violations
CREATE POLICY "Anyone can view zoning violations" ON zoning_violations FOR SELECT USING (true);

CREATE POLICY "Authenticated users can report violations" ON zoning_violations FOR INSERT 
WITH CHECK (auth.uid() = reporter_id OR is_anonymous = true);

CREATE POLICY "Users can update own violations or admins can manage" ON zoning_violations FOR UPDATE 
...
-- Enhance job_postings to allow user-created jobs
...
-- Enable RLS on issue updates
ALTER TABLE issue_updates ENABLE ROW LEVEL SECURITY;

-- Create policies for issue updates
CREATE POLICY "Users can view issue updates" ON issue_updates FOR SELECT USING (true);

CREATE POLICY "Issue reporters can add updates" ON issue_updates FOR INSERT 
WITH CHECK (EXISTS (
  SELECT 1 FROM civic_issues 
  WHERE civic_issues.id = issue_updates.issue_id 
  AND civic_issues.reporter_id = auth.uid()
));

CREATE POLICY "Users can update own updates" ON issue_updates FOR UPDATE 
...
-- Enable RLS on violation updates
ALTER TABLE violation_updates ENABLE ROW LEVEL SECURITY;

-- Create policies for violation updates
CREATE POLICY "Users can view violation updates" ON violation_updates FOR SELECT USING (true);

CREATE POLICY "Violation reporters can add updates" ON violation_updates FOR INSERT 
WITH CHECK (EXISTS (
  SELECT 1 FROM zoning_violations 
  WHERE zoning_violations.id = violation_updates.violation_id 
  AND zoning_violations.reporter_id = auth.uid()
));

-- Enhance collaboration_groups with better location support
...
-- Enable RLS on stakeholder profiles
ALTER TABLE stakeholder_profiles ENABLE ROW LEVEL SECURITY;

-- Create policies for stakeholder profiles
CREATE POLICY "Anyone can view stakeholder profiles" ON stakeholder_profiles FOR SELECT USING (true);

CREATE POLICY "Users can manage own stakeholder profile" ON stakeholder_profiles FOR ALL 
USING (auth.uid() = user_id);
...
-- Enable RLS on cms job postings
ALTER TABLE cms_job_postings ENABLE ROW LEVEL SECURITY;

-- Create policies for cms job postings
CREATE POLICY "Anyone can view published jobs" ON cms_job_postings FOR SELECT 
USING (status = 'published');

CREATE POLICY "Admins can manage job postings" ON cms_job_postings FOR ALL 
USING (EXISTS (
  SELECT 1 FROM user_profiles 
  WHERE user_profiles.id = auth.uid() 
  AND user_profiles.user_type = 'admin'
...
ALTER TABLE issue_updates REPLICA IDENTITY FULL;
ALTER TABLE violation_updates REPLICA IDENTITY FULL;
ALTER TABLE zoning_violations REPLICA IDENTITY FULL;
ALTER TABLE stakeholder_profiles REPLICA IDENTITY FULL;
ALTER TABLE cms_job_postings REPLICA IDENTITY FULL;

-- Add to realtime publication
ALTER PUBLICATION supabase_realtime ADD TABLE issue_updates;
ALTER PUBLICATION supabase_realtime ADD TABLE violation_updates;
ALTER PUBLICATION supabase_realtime ADD TABLE zoning_violations;
ALTER PUBLICATION supabase_realtime ADD TABLE stakeholder_profiles;
ALTER PUBLICATION supabase_realtime ADD TABLE cms_job_postings;

-- Insert sample data
INSERT INTO stakeholder_categories (name, description, relevance_keywords, issue_categories)
VALUES 
  ('Local Businesses', 'Commercial establishments in the area', ARRAY['business', 'commerce', 'economic'], ARRAY['infrastructure', 'zoning']),
  ('Resident Associations', 'Community groups and resident committees', ARRAY['residents', 'community', 'neighborhood'], ARRAY['all']),
  ('Government Agencies', 'County and national government departments', ARRAY['government', 'official', 'policy'], ARRAY['all']),
  ('NGOs & CSOs', 'Non-governmental organizations and civil society', ARRAY['advocacy', 'nonprofit', 'community'], ARRAY['social', 'environment']),
  ('Media Outlets', 'Local and national media organizations', ARRAY['media', 'journalism', 'news'], ARRAY['all']),
  ('Religious Organizations', 'Churches, mosques, and other faith-based groups', ARRAY['faith', 'religious', 'spiritual'], ARRAY['social', 'community']),
  ('Educational Institutions', 'Schools, colleges, and training centers', ARRAY['education', 'school', 'training'], ARRAY['infrastructure', 'social'])
ON CONFLICT (name) DO NOTHING;

-- Sample zoning violations
